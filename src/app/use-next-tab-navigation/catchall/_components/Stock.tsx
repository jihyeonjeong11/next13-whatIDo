  ...
  return (
    <div className="bg-gray-50 min-h-screen p-8 text-slate-900">
      ...
      <header className="mb-8 border-b pb-4">
        <h1 className="text-4xl font-extrabold">AAPL: Apple Inc. Historical Analysis</h1>
        <p className="text-gray-500">Comprehensive 10-Year Performance & Technical Indicators</p>
      </header>
            ...
            <tbody>
              {Stockdata.map((row, idx) => (
                <tr key={idx} className="hover:bg-blue-50 transition-colors border-b text-sm">
                  ...
                  <td className="p-2 border font-mono text-xs">{row.volume.toLocaleString()}</td>
          ...
          {Array.from({ length: 20 }).map((_, i) => (
            <div key={i}>
              ...
              <p className="text-sm text-gray-600">
                ...
                page. When React hydrates this page, it must parse every single one of these
                paragraphs, which increases the Total Blocking Time (TBT). This is a common pattern
                in e-commerce and financial detail pages that leads to poor Core Web Vitals.
              </p>
            </div>
          ))}
        </div>
